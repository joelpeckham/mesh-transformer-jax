...
if __name__ == "__main__":
    ...
    with jax.experimental.maps.mesh(devices, ('dp', 'mp')):
        ...
        import json
        testFiles = []
        # For each file in the test_files folder
        for fileName in os.listdir(args.test_files):
            filePath = os.path.join(args.test_files, fileName)
        ...
        for article in testFiles[:limit]:
            ...
            if len(context.split()) > 1010:
                print(f"Skipping {article['title']} because it is too long")
                continue
            ...
            output = network.generate(batched_tokens, length, 15, {"top_p": np.ones(total_batch) * 0.9,
